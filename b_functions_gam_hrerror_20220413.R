...
form.ti<-as.formula(area~
                      intsem
                    +s(log10(nbobs),k=knbobs,bs=spl[1],by=intsem)
                    +s(trap.d,k=5,bs=spl[2],by=intsem)
                    +s(area.c,k=5,bs=spl[3],by=intsem)
                    +ti(log10(nbobs),trap.d,k=c(knbobs,5),bs=spl[c(1,2)],by=intsem)
                    +ti(log10(nbobs),area.c,k=c(knbobs,5),bs=spl[c(1,3)],by=intsem)
                    +ti(trap.d,area.c,k=c(5,5),bs=spl[c(2,3)],by=intsem)
                    +ti(log10(nbobs),area.c,trap.d,k=c(knbobs,5,5),bs=spl[c(1,2,3)],by=intsem)
)
...
pred_gam <- function(modell, newdata, varint, specials, nms,alpha = 0.05,
  ...
  preds<-map2(newdata,varint,function(data,vint){
    ...
    # data<-data[which(data$intsem%in%modell$model$intsem),]
    # data$intsem<-factor(data$intsem,levels=levels(modell$model$intsem))
    # 
    # nms<-c("area.c","trap.d","nbobs")
    # varint<-list(c("intsem","area.c"),c("intsem","trap.d"),c("intsem","nbobs"))
    # vint<-varint[[3]]
    # specials<-c("[()]","log10")
